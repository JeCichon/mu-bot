...
// ─── Fortune Templates ────────────────────────────────────────────────────────
// Each template takes 3 cards (A, B, C) and returns a fortune string.
// Fields available: timeframe, command, subject, verb, adv, adj, noun
...
const MU_FORTUNE_INTROS = [
  "*Mu reaches into the archive and produces a folded note.*",
  "*A card slides across the library table. Mu taps it once.*",
  "*Mu turns, holds something toward you, says nothing.*",
  "*From the stacks, Mu produces this. He does not explain it.*",
  "*Mu has been watching. He offers what he sees.*",
  "*The great librarian bows slightly and extends this to you.*",
];
...
// ─── Sentence Builder (for Hey Mu / Sup Mu / askmu) ──────────────────────────
...
client.once('clientReady', async () => {
  ...
  try {
    ...
    console.log('✓ Commands registered: /draw, /draw3, /askmu, /fortune');
  } catch (err) {
  ...
  // Daily fortune — 8am UTC, one random non-bot member
  cron.schedule('0 8 * * *', async () => {
    try {
      ...
      await channel.send({
        content: `<@${member.id}>`,
        embeds: [buildFortuneEmbed(cards, member)]
      });
